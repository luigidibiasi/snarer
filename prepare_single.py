# This script prepares input dataset for MATHLAB
#
#
#
...
def PARSE_STRING(string):
    ...
    print("LENGTH: " + str(cprot));
    entire = open("output.txt","w")
    
    #COMPUTE MEAN VALUES FOR EACH DESCRIPTORS (TO TALK WITH ALESSIA HERE)
    ARGP820102 = 0.0
    CHAM830101 = 0.0
    CHAM830107 = 0.0
    CHAM830108 = 0.0
    ...
    str(ZIMJ680102) + '\t' +
    str(ONEK900102) + '\t' +
    str(steric_parameter)+'\t'+str(polarizability)+'\t'+str(volume)+'\t'+str(isoelectric_point)+'\t'+str(helix_probability)+'\t'+str(sheet_probability)+'\t'+str(hydrophobicity)+'\t?\n');
